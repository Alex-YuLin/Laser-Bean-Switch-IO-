...
namespace GalvoScan.Hardware.Key.Sentinel
{
    class Key_Sentinel:IKey
    {
        SentineKey gSentineKey;
        SentineKey.FeatureInfo gInfo;
        ...
        public override RetErr Open()
        {
            ...
            try
            {
                ...
                // init & Get Key info
                ret = gSentineKey.CheckLincense_Software(out _HWtype, out gInfo);
                if (!ret.flag)
        ...
        {
            string ret = gInfo.Year.ToString() + " / " + gInfo.Month.ToString("d2") + " / " + gInfo.Day.ToString("d2");
            return ret;
        }
    }
    ...
    class SentineKey
    {
        ...
        public RetErr CheckLincense_Software(out HWType hwType, out FeatureInfo data)
        {
            RetErr ret = new RetErr();
